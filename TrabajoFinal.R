...
qfiltrado_porcentaje<-dbGetQuery(conexion,"select departamento_id, porcentaje 
                                 from partosadolescentes.porcentaje_profesionales as pp 
                                 where pp.porcentaje is null")
View(qfiltrado_porcentaje)#encontrar cantidad de nulos por años
...
###########################Para jovenes embarazadas####################################
#Primero reemplazamos los datos de la columna porcentajes que son NA por 0
df_tabla_porcentaje_adolescentes_emb$porcentaje[is.na(df_tabla_porcentaje_adolescentes_emb$porcentaje)] <- 0
#MEDIA aritmetica del procentaje de adolescentes_emb
mean(df_tabla_porcentaje_adolescentes_emb$porcentaje)

#MAXIMO del porcentaje de adolescentes_emb
max(df_tabla_porcentaje_adolescentes_emb$porcentaje)

#MINIMO del porcentaje de adolescentes_emb
min(df_tabla_porcentaje_adolescentes_emb$porcentaje)

#CUARTILES del porcentaje de adolescentes_emb
q_ados_emb<-quantile(df_tabla_porcentaje_adolescentes_emb$porcentaje, prob=c(0,0.25,0.5,0.75,1))

#PERCENTILES del porcentaje de adolescentes_emb TODOS
quantile(df_tabla_porcentaje_adolescentes_emb$porcentaje, prob=seq(0, 1, length = 101))
...
#Adolescentes
ggplot(df_emb_fecha_m4_dep_m3, aes(fecha, df_emb_fecha_m4_dep_m3$porcentaje, fill = df_emb_fecha_m4_dep_m3$Departamento)) +
  geom_bar(stat = "identity", position = "dodge") + coord_flip() + labs(x= "Fecha", y = "Porcentaje embarazo adolescente", fill="Departamento")

ggplot(qjoin_porcentaje_emb, aes(qjoin_porcentaje_emb$departamento, qjoin_porcentaje_emb$porcentaje, fill = qjoin_porcentaje_emb$fecha)) +
  geom_bar(stat = "identity", position = "dodge") + coord_flip() + labs(x= "Departamentos", y = "Porcentaje Profesionales", fill="Año")

ggplot(data=qjoin_porcentaje_emb, aes(x=qjoin_porcentaje_emb$fecha, y=qjoin_porcentaje_emb$porcentaje)) + 
...
boxplot(formula = df_prof_fec_dep_4p$porcentaje_profesional ~ df_prof_fec_dep_4p$Departamento, data =  df_prof_fec_dep_4p,  xlab = "Departamento", ylab = "Porcentaje de profesionales")
boxplot(formula = qjoin_porcentaje_emb$porcentaje ~ qjoin_porcentaje_emb$departamento, data =  qjoin_porcentaje_emb,  xlab = "Departamento de Ucayali", ylab = "Porcentaje de profesionales")
...
total<-(a+b+c+d+e+f+g)
#Porcentaje por cada fecha presentes en el KNN
a*100/total
b*100/total
c*100/total
d*100/total
...
regresion(paraKNN$porcentaje_profes,paraKNN$porcentaje_adoles,nx=14)
plot(paraKNN$porcentaje_profes,paraKNN$porcentaje_adoles)
#######################################################
#Exportar archivos a excel- LA TABLA PROFESIONAL
